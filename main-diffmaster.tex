...
%DIF DEL main-oldtmp-36854.tex   Sun Aug 12 17:22:09 2018
%DIF ADD main.tex                Sun Aug 12 17:21:54 2018
...
\chapter{Uvod}
\DIFdelbegin \DIFdel{večanje rač resoursev (cpu, mem) potreba }\DIFdelend \DIFaddbegin 

\DIFadd{Diplomsko delo je razdeljeno na tri večja poglavja. V prvem poglavju bom predstavil razlog za nastanek NewSQL podatkovnih baz, opisal njihove lastnosti, razdelitev in najpogosteje uporabljene mehanizme ter pristope.
}

\DIFadd{V drugem poglavju bom opisal podatkovno bazo CockroachDB. Opisal bom njeno zgodovino in idejno osnovo. Poglobil se bom v arhitekturo podatkovne baze CockroachDB in predstavil nekaj ključinh mehanizmov kateri omogočajo standardni ANSI SQL vmesnik, ACID transakcije, konsistenco, replikacijo, visoko dostopnost in enostavnost z upravljanjem. Kasneje bom predstavil še ostale lastnosti kot so SQL vmesnik, poslovna licenca ter podprta orodja, gonilniki in ORM-ji.
}
...
\hl{TODO:}
\begin{itemize}
    \item \DIFadd{Premik v oblak
    }\item \DIFadd{Večja izkoriščenost računalniških resursev
    }\item \DIFadd{Globalna replikacija in visoka dostopnost
    }\item \DIFadd{Potreba }\DIFaddend po spremembi arhitekture
\DIFdelbegin \DIFdel{.
...
Na uradni spletni dokumentaciji je za jezike Go, Java, .NET, C++, NodeJS, PHP, Python, Ruby, Rust, Clojure ter Elixir objavljen seznam podprtih gonilnikov in ORMjev. Za te CockroachLabs v času pisanja zagotavlja le beta podporo \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
\cite{kaur2017performance, Benchmarking-GCS-CRDB-NuoDB, CRDB-tpcc-vs-aurora}}\hspace{0pt}%DIFAUXCMD
...
\DIFadd{Glede na znanje katerega sem pridobil skozi diplomsko delo, menim, da je podatkovna baza CockroachDB primerna za nove transakcijsko usmerjene (OLTP) aplikacije, katere zahtevajo visoko konsistenoc in dostopnost. Aplikacije katere ciljajo globalni trg, kjer je čas do trga zelo pomemben in kjer si ne moremo privoščiti velikih stroškov vzdrževanja. Pred odločitvijo moramo oceniti še podprto sintakso SQL jezika \mbox{%DIFAUXCMD
...
\DIFadd{V prihodnje bi bilo zanimivo raziskati in primerjati še zelo podobno in prav tako odprto kodno podatkovno bazo TiDB \mbox{%DIFAUXCMD
