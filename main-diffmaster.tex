...
%DIF DEL main-oldtmp-4588.tex   Fri Aug 31 09:01:21 2018
...
\DIFdel{strukturiran povpraševalni jezik za delo z relacijskimi podatkovnimi bazami
        }%DIFDELCMD < \\
%DIFDELCMD <     {\bf %%%
...
Za vsak obseg obstaja \DIFdelbegin \DIFdel{Raft skupina }\DIFdelend \DIFaddbegin \DIFadd{skupina Raft}\DIFaddend , kjer je eno vozlišče, ki vsebuje repliko, označeno kot "vodja". Vodja je izvoljen in koordinira vse pisalne zahtevke za določen obseg. V idealnih pogojih je vodja \DIFdelbegin \DIFdel{Raft skupine }\DIFdelend \DIFaddbegin \DIFadd{skupine Raft }\DIFaddend tudi najemnik obsega (angl. leaseholder).
...
\DIFdelend \DIFaddbegin \DIFadd{(podatki v JSON obliki, brez presledkov in podvojenih ključev, predstavljeni v binarnem zapisu) in }\DIFaddend \texttt{UUID} (128-bitna heksadecimalna vrednost)
\end{itemize}
...
\begin{itemize}
    ...
    Orodje je zelo konfigurabilno, vendar pa ne omogoča \DIFdelbegin \DIFdel{v naprej }\DIFdelend \DIFaddbegin \DIFadd{vnaprej }\DIFaddend definiranih obremenitvenih testov. Zaradi kompleksnosti je težko za uporabo.
...
Ker tehnologije Docker še nismo dobro poznali, smo imeli v fazi prototipiranja težave pri zmogljivosti. Izkazalo se je, da je bil ključen problem v tem, da \DIFaddbegin \DIFadd{sprva }\DIFaddend nismo uporabili podatkovnih nosilcev (angl. data volumes) \cite{docker-storage-layers}.
...
Citus je razširitev za podatkovno bazo PostgreSQL \cite{citus}. Omogoča enostavno horizontalno skaliranje podatkov za \DIFdelbegin \DIFdel{več najemniške }\DIFdelend \DIFaddbegin \DIFadd{večnajemniške }\DIFaddend (angl. multi tenant) aplikacije in obdelavo analitičnih podatkov v realnem času (angl. real time analytics). Uporaba razširitve Citus ni primerna, če:
\begin{itemize}
...
Za Citus \DIFdelbegin \DIFdel{podatkovno bazo }\DIFdelend je postopek nekoliko daljši. Ker ne moremo neposredno s porazdeljene tabele \texttt{usertable} kopirati v lokalno tabelo \texttt{ext}, moramo najprej kreirati začasno tabelo, podatke napolniti in potem iz začasne tabele podatke prestaviti v novo tabelo \texttt{ext}. To smo storili z naslednjo skripto:
...
CockroachDB 3 \DIFdelbeginFL \DIFdelFL{vozlišče  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{vozlišča  }\DIFaddendFL & (prekinjeno po 1 minuti)\\
\end{tabular}
\end{center}
...
\end{center}
\caption{Prikazuje primerjavo prepustnosti in latenc glede na vrsto obremenitve med podatkovno bazo \DIFdelbeginFL \DIFdelFL{Citus in }\DIFdelendFL CockroachDB \DIFaddbeginFL \DIFaddFL{in Citus}\DIFaddendFL . Graf (7) je zaradi lažje primerjave odrezan. Iz grafov je razvidno, da je \DIFdelbeginFL \DIFdelFL{podatkovna baza }\DIFdelendFL Citus v primerjavi s \DIFaddbeginFL \DIFaddFL{podatkovno bazo }\DIFaddendFL CockroachDB veliko bolj \DIFdelbeginFL \DIFdelFL{stabilna}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{stabilen}\DIFaddendFL , kar se odraža v razponu med največjo in najmanjšo vrednostjo.}
...
\section{Ugotovitve}
Zmogljivostna analiza je pokazala, da se \DIFdelbegin \DIFdel{podatkovna baza }\DIFdelend Citus pri večini obremenitev, katere smo testirali, odziva bistveno bolje kot podatkovna baza CockroachDB.
...
S slike \ref{img_ycsb_results_bnw_comparison} je razvidno, da je podatkovna baza CockroachDB v primerjavi s \DIFdelbegin \DIFdel{podatkovno bazo }\DIFdelend Citus manj stabilna. To se odraža na posameznih grafih, saj so rezultati meritev pri podatkovni bazi CockroachDB veliko bolj razpršeni.

Če primerjamo spremembe sočasnih niti, pri katerih sta bazi dosegali maksimalno prepustnost (slika \ref{img_ycsb_results_max_throughput}), opazimo naslednje. Podatkovna baza CockroachDB ob skaliranju na tri vozlišča lahko v povprečju obdela 5,4 več sočasnih niti, \DIFdelbegin \DIFdel{podatkovna baza }\DIFdelend Citus pa kar 10,8 manj.
...
Na podlagi teh ugotovitev smo pripravili optimistično oceno rasti povprečne prepustnosti. Obe bazi se v idealnih pogojih in do neke mere skalirata linearno, kar je prikazano na sliki \ref{img_ycsb_results_scaling_throughptu_prediction}. Vendar pa se pri PostgreSQL konfiguraciji vsi odjemalci neposredno povezujejo na eno vozlišče (koordinatorja), ki je ozko grlo (točka A), saj bo ob večanju v neki točki prišlo do zasičenosti resursov \cite{Citus-add-coordinator}. V neki točki (točka B) pa bo CockroachDB presegel prepustnost podatkovne baze \DIFaddbegin \DIFadd{PostgreSQL z nameščeno razširitvijo }\DIFaddend Citus.

\begin{figure}[H]
\begin{center}
\includegraphics[width=0.7\textwidth]{resources/scaling-throughput-prediction-v2.png}
\end{center}
\caption{Optimistična ocena rasti povprečne prepustnosti. Pri \DIFdelbeginFL \DIFdelFL{podatkovni bazi }\DIFdelendFL Citus se v točki A linearna rast konča. V točki B pa ima CockroachDB že večjo prepustnost. Graf je le simboličen in ne predstavlja realnih meritev.}
