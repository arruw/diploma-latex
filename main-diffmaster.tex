...
%DIF DEL main-oldtmp-12472.tex   Wed Aug 15 11:19:36 2018
%DIF ADD main.tex                Wed Aug 15 11:17:03 2018
...
\DIFadd{Sledijo sklepne ugotovitve, kjer bom povzel diplomsko delo in izpostavil zanimive ugotovitve. Naštel bom nekaj produktov kjer je ta baza že v uporabi. Za zaključek bom še predlagal nekaj odprtih vprašanj in idej za nadaljnje raziskovanje.
}
...
Na podlagi teh ugotovitev sem pripravil optimistično oceno rasti povprečne prepustnosti. Obe bazi se v idealnih pogojih in do neke mere skalirata linearno, kar je prikazano na sliki \ref{img_ycsb_results_scaling_throughptu_prediction}. Vendar pa se pri Postgres konfiguraciji vsi odjemalci direktno povezujejo na eno vozlišče (koordinatorja), to vozlišče je ozko grlo, saj bo ob večanjem v neki točki prišlo do zasičenosti \DIFdelbegin \DIFdel{resursev }\DIFdelend \DIFaddbegin \DIFadd{resursov }\DIFaddend \cite{Citus-add-coordinator}.
...
V diplomskem delu sem pregledal lastnosti  ter najpogosteje uporabljene mehanizme in pristope pri implementacijah novih arhitektur NewSQL podatkovnih baz. \DIFdelbegin \DIFdel{Nato sem opisal }\DIFdelend \DIFaddbegin \DIFadd{Opisal sem }\DIFaddend arhitekturo in lastnosti izbrane NewSQL podatkovne baze CockroachDB. Podatkovno bazo CockroachDB sem kasneje praktično testiral. Izvedel sem primerjalno analizo zmogljivosti med podatkovno bazo CockroachDB in Postgres (z nameščeno razširitvijo Citus). Veliko časa sem vložil v postavitev testnega okolja, \DIFdelbegin \DIFdel{pazil sem }\DIFdelend \DIFaddbegin \DIFadd{pozoren sem bil}\DIFaddend , da sem odpravil čim več spremenljivk, ki bi lahko \DIFdelbegin \DIFdel{uplivale ne }\DIFdelend \DIFaddbegin \DIFadd{vplivale na }\DIFaddend testne scenarije. Testno okolje je bilo sestavljeno \DIFdelbegin \DIFdel{z }\DIFdelend \DIFaddbegin \DIFadd{s }\DIFaddend štirih starejših računalnikov, kateri so bili povezani preko gigabitnega omrežja, na njih pa je tekel Ubuntu Server in Docker. Testno okolje sem v začetku nadziral preko Telegraf agenta, InfulxDB podatkovne baze za \DIFdelbegin \DIFdel{sehranjevanje }\DIFdelend \DIFaddbegin \DIFadd{shranjevanje }\DIFaddend časovnih podatkov in Grafane za vizualizacijo podatkov. Kljub trudu, ki je bil vložen v postavitev testnega okolja, bi le tega lahko še izboljšal. Za boljšo primerljivost zmogljivostnih metrik, bi morala imeti vsa vozlišča enako strojno opremo, testirati pa bi moral še konfiguracije z več kot tremi vozlišči. Izvedbo testiranja sem avtomatiziral\DIFdelbegin \DIFdel{, z }\DIFdelend \DIFaddbegin \DIFadd{. Z }\DIFaddend orodjem YCSB sem izvajal različne zmogljivostne teste, \DIFdelbegin \DIFdel{razultate }\DIFdelend \DIFaddbegin \DIFadd{rezultate }\DIFaddend pa sem shranjeval v csv datoteko in jih kasneje obdelal \DIFdelbegin \DIFdel{z }\DIFdelend \DIFaddbegin \DIFadd{s }\DIFaddend programom Microsoft Excel.

Primerjalna analiza zmogljivosti je pokazala, da je podatkovna baza Cock\-roachDB kljub enostavnim obremenitvam, katere vrši orodje YCSB, v večini primerov slabša od Postgres podatkovne baze. V \DIFdelbegin \DIFdel{porvprečju }\DIFdelend \DIFaddbegin \DIFadd{povprečju }\DIFaddend je imel Cock\-roachDB večjo latenco in manjšo prepustnost, poleg tega pa je z večanjem sočasnih povezav zmogljivost padala bistveno hitreje kot pri podatkovni bazi Postgres. Kljub slabšim \DIFdelbegin \DIFdel{rezultatov }\DIFdelend \DIFaddbegin \DIFadd{rezultatom }\DIFaddend zmogljivostne analize, se moramo zavedati, da je težko pošteno primerjati dve \DIFdelbegin \DIFdel{zele }\DIFdelend \DIFaddbegin \DIFadd{zelo }\DIFaddend različni podatkovni bazi. Podatkovna baza Postgres je bistveno starejša in zato bolj stabilna \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{ter }\DIFaddend optimizirana. CockroachDB pa je na trgu šele dobri dve leti in je trenutno še vedno bolj funkcionalno usmerjena. Za smiselno odločitev pri izbiri podatkovne baze \DIFdelbegin \DIFdel{pa }\DIFdelend moramo poleg zmogljivostnih metrik upoštevati tudi druge, kot so poizvedovalni jezik, podpora transakcijam, zrelost sistema, skupnost, zmogljivost, težavnost postavitve ter vzdrževanja, \DIFdelbegin \DIFdel{oslate }\DIFdelend \DIFaddbegin \DIFadd{ostale }\DIFaddend lastnosti specifične za vsako podatkovno bazo, ...
...
Kljub temu, da je CockroachDB relativno nova podatkovna baza\DIFaddbegin \DIFadd{, }\DIFaddend jo nekatera podjetja že uporabljajo. CockroachLabs poizkuša s podjetji sodelovati, nekatera podjetja so tudi \DIFdelbegin \DIFdel{prisplevala }\DIFdelend \DIFaddbegin \DIFadd{prispevala }\DIFaddend del funkcionalnosti. Podjetja katera so se odločila za uporabo podatkovne baze CockroachDB, ciljajo globalni trg, poleg tega pa želijo poenostaviti postavitev in vzdrževanje, tako v oblak, kako tudi na svoji infrastrukturi.

\begin{itemize}
    \item \textbf{Baidu} uporablja CockroachDB pri dveh novih aplikacijah, ki sta predhodno uporabljale MySQL podatkovno bazo. Ti dve aplikaciji obsegata približno 2TB podatkov in ustvarita \DIFdelbegin \DIFdel{prbližno }\DIFdelend \DIFaddbegin \DIFadd{približno }\DIFaddend 50M zapisov na dan.
    \item \textbf{Kindred} se ukvarja z igralništvom, z letom 2014 pa so začeli s prehodom na globalni trg. Imajo kompleksen ekosistem \DIFaddbegin \DIFadd{z }\DIFaddend več kot 200 mikrostoritev\DIFdelbegin \DIFdel{, ta }\DIFdelend \DIFaddbegin \DIFadd{. Ta }\DIFaddend arhitektura omogoča elastičnost poleg tega pa mora biti skoraj v celoti avtonomna.
    \item \textbf{Tierion} ponuja "Blockchain proof engine" rešitev. Ovrednotili so več opcij\DIFaddbegin \DIFadd{, }\DIFaddend na koncu pa so se odločili za CockroachDB.
    \item \textbf{Heroic} Labs je hitro rastoč startup, ukvarja se \DIFdelbegin \DIFdel{z }\DIFdelend \DIFaddbegin \DIFadd{s }\DIFaddend skalabilnimi zalednimi sistemi v igralniški industriji.
    \item \DIFdelbegin \DIFdel{Gorgias }\DIFdelend \DIFaddbegin \textbf{\DIFadd{Gorgias}} \DIFaddend ponuja platformo za pomoč uporabnikom.
\end{itemize}
...
\DIFdel{Kaj bi bilo smiselno }\DIFdelend %DIF > Podatkovna baza CockroachBD idejno izhaja iz Googlove podatkovne baze Spanner. Podobno je s podatkovno bazo TiDB, katera je v nekaterih pogledih zelo podobna podatkovni bazi CockroachBD. TiDB je prav tako sestavljena z več plasti. Za razliko s CockroachDB, namestitev obsega tri ločene komponente TiDB (vmesnik med SQL in KV operacijami), TiKV (shramba podatkov) in PD (meta podatki in generiranje časovnih oznak).
\DIFaddbegin 
