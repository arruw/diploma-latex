...
%DIF DEL main-oldtmp-49145.tex   Tue Aug 14 07:39:57 2018
%DIF ADD main.tex                Tue Aug 14 07:39:21 2018
...
\chapter{Uvod}
\DIFdelbegin \DIFdel{večanje rač resoursev (cpu, mem) potreba po spremembi arhitekture}\DIFdelend \DIFaddbegin \DIFadd{Trandi kažejo, da se vse več računalniške infrastrukture premika v oblak, s tem pa se prilagajajo in razvijajo tudi nove tehnologije, ki so temu bolj primerne. To se odraža tudi pri podatkovnih bazah. Iz starih monolitičnih relacijskih podatkovnih baz, katere je še zlasti težko vzdrževati v porazdeljenih okoljih, so se razvile ne relacijske podatkovne baze in kasneje nove relacijske oziroma NewSQL podatkovne baze. NewSQL relacijske podatkovne baze so tako prilagojene za oblak, zagotavljajo visoko konsistenco in razpoložljivost, poleg tega pa nudijo tudi boljši izkoristek računalniških virov.
}

\DIFadd{Podatkovna baza CockroachDB je ena izmed NewSQL podatkovnih baz. Z raziskovalnega področja je zanimiva ker je v celoti odprtokodna, idejno pa izhaja z Googlove podatkovne baze Spanner, je zelo enostavna za uporabo in je skoraj v celoti avtomatizirana. Na prvi pogled vsebuje vse, kar bi pričakovali od transakcijsko usmerjene (OLTP) podatkovne baze}\DIFaddend . 

\DIFaddbegin \DIFadd{Diplomsko delo je razdeljeno na tri večja poglavja. V prvem poglavju bom predstavil razlog za nastanek NewSQL podatkovnih baz, opisal njihove lastnosti, razdelitev in najpogosteje uporabljene mehanizme ter pristope.
}

\DIFadd{V drugem poglavju bom opisal podatkovno bazo CockroachDB. Opisal bom njeno zgodovino in idejno osnovo. Poglobil se bom v arhitekturo podatkovne baze CockroachDB in predstavil nekaj ključinh mehanizmov kateri omogočajo standardni SQL vmesnik, transakcije, konsistenco, replikacijo, visoko razpoložljivost in enostavnost z upravljanjem. Kasneje bom predstavil še ostale lastnosti kot so SQL vmesnik, poslovna licenca ter podprta orodja, gonilniki in ORM-ji.
}

\DIFadd{V tretjem poglavju bom opisal celoten postopek izvedbe primerjalne analize zmogljivosti med podatkovno bazo CockroachDB in Citus. Opisal bom hipoteze, testno infrastrukturo, izbiro orodij za izvedbo zmogljivostnih analiz, pripravo podatkov, izvedbo analize. Na koncu bom predstavil rezultate in komentiral hipoteze.
}

\DIFadd{Na koncu sledijo sklepne ugotovitve, kjer bom povzel diplomsko delo. Naštel bom nakej produktov kjer je ta baza že v uporabi. Za zaključek bom še predlagal nekaj odprtih vprašanj in idej za nadalnje raziskovanje.
}
...
Ker \DIFdelbegin \DIFdel{imajo IMDB podatkovne baze zelo majhne stroške operacij, lahko }\DIFdelend \DIFaddbegin \DIFadd{so bralne in pisalne operacije pri IMDB podatkovnih bazah, v primerjavi z tradicionalnimi relacijskimi podatkovnimi bazami, relativno veliko hitrejše, lahko te }\DIFaddend bolje izkoristijo procesorsko moč\DIFdelbegin \DIFdel{in se }\DIFdelend \DIFaddbegin \DIFadd{. Posledično se lahko }\DIFaddend uporabijo za poslovno analitiko na živih podatkih ter hkrati omogočajo zelo visoko pisalno prepustnost. Te procese označimo kot hibridno transakcijsko analitične procese oziroma HTAP (angl. hybrid transaction/analytical processing) \cite{NewSqlInMemoryAnalytics}.
...
\begin{enumerate}
    \item \textbf{Delitev glede na interval:}\\Glede na postavljene povezane \DIFdelbegin \DIFdel{ne-prekrivajoče }\DIFdelend \DIFaddbegin \DIFadd{ne prekrivajoče }\DIFaddend intervale se zapisi delijo na \DIFdelbegin \DIFdel{particij}\DIFdelend \DIFaddbegin \DIFadd{particije}\DIFaddend . Ta pristop je uporaben predvsem kjer so podatki logično strukturirani v intervale, na primer po letih, kvartalih, mesecih...
...
Idealno podatkovna baza \DIFdelbegin \DIFdel{z }\DIFdelend \DIFaddbegin \DIFadd{iz }\DIFaddend poizvedbe prepozna katerih particij se poizvedba dotika ter nato izvede poizvedbo porazdeljeno preko teh particij. Dobljene rezultate združi in vrne en rezultat \cite{Pavlo2016Sep}.

Nekatere izmed NewSQL podatkovnih baz omogočajo migracijo podatkov med particijami med izvajanjem podatkovne baze. Ta mehanizem je potreben, ker nekatere \DIFdelbegin \DIFdel{particij }\DIFdelend \DIFaddbegin \DIFadd{particije }\DIFaddend rastejo hitreje kot druge. Če želimo, da podatkovna baza deluje optimalno\DIFaddbegin \DIFadd{, }\DIFaddend potrebujemo podatke ponovno razdeliti enakomerno. Ta pristop ni nov, uporablja ga večina NoSQL podatkovnih baz, je pa bolj kompleksen\DIFaddbegin \DIFadd{, }\DIFaddend saj NewSQL podatkovne baze 
potrebujejo zagotavljati ACID lastnosti \cite{Pavlo2016Sep}.

\section{Replikacija}

Najboljši način za doseganje visoke razpoložljivosti je uporaba replikacije. Večina sodobnih podatkovnih baz podpira \DIFdelbegin \DIFdel{neko vrsto }\DIFdelend \DIFaddbegin \DIFadd{nek pristop }\DIFaddend replikacije \cite{Pavlo2016Sep}. V osnovi \DIFdelbegin \DIFdel{obstaja dva pristopa replikacije}\DIFdelend \DIFaddbegin \DIFadd{popznamo dva pristopa repliciranja}\DIFaddend :
\begin{enumerate}
    \item \textbf{\DIFdelbegin \DIFdel{Aktivna }\DIFdelend \DIFaddbegin \DIFadd{Sinhrona }\DIFaddend replikacija}:\\Imenujemo jo tudi aktivno-aktivna (angl. active-active) \DIFdelbegin \DIFdel{oziroma več-gospodarska (angl. multi-master) replikacija\mbox{%DIFAUXCMD
\cite{HighAvailabilityDatabase}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{replikacija}\DIFaddend . Pri tem pristopu vse replike sočasno obdelajo zahtevek \cite{Pavlo2016Sep}.
\DIFaddbegin 

    \DIFaddend \item \textbf{\DIFdelbegin \DIFdel{Pasivna }\DIFdelend \DIFaddbegin \DIFadd{Asinhrona }\DIFaddend replikacija}:\\Imenujemo jo tudi aktivno-pasivna (angl. active-passive) \DIFdelbegin \DIFdel{oziroma gospodar-suženj (angl. master-slave) replikacija\mbox{%DIFAUXCMD
\cite{HighAvailabilityDatabase}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{replikacija}\DIFaddend . Pri tem pristopu se zahtevek najprej obdela na eni repliki, nato pa se sprememba posreduje ostalim replikam. Ta pristop implementira večina NewSQL podatkovnih baz, saj zaradi ne deterministične \DIFdelbegin \DIFdel{sočasnosti }\DIFdelend \DIFaddbegin \DIFadd{sočas\-nosti }\DIFaddend ni mogoče izvesti zahtevkov v pravilnem vrstnem redu na vseh replikah \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Pavlo2016Sep}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Pavlo2016Sep, harding2017evaluation}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
\end{enumerate}

\DIFdelbegin %DIFDELCMD < \hl{TODO:}
%DIFDELCMD < %%%
\DIFdel{Sinhrona in asinhrona replikacija
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend V visoko konsistentnih podatkovnih bazah morajo biti \DIFdelbegin \DIFdel{transakcije }\DIFdelend \DIFaddbegin \DIFadd{replikacija }\DIFaddend potrjene na vseh replikah\DIFaddbegin \DIFadd{, }\DIFaddend šele takrat se smatra, da je \DIFdelbegin \DIFdel{transakcija potrjena \mbox{%DIFAUXCMD
\cite{NewSqlInMemoryAnalytics}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{replikaija uspešna \mbox{%DIFAUXCMD
\cite{NewSqlInMemoryAnalytics}}\hspace{0pt}%DIFAUXCMD
. Pri distribuiranih podatkovnih bazah, katere uporabljajo soglasni algoritem (angl. consensus algorithm), je dovolj, da replikacijo potrdi večina replik.
}\DIFaddend 

\DIFaddbegin \DIFadd{Ker so NewSQL podatkovne baze prilagojene za oblak in namestitve med katerimi so velike geografske razlike, podpirajo tudi optimizirano replikacijo preko WAN (angl. wide-area network) omrežji.
}

\DIFaddend \section{Obnova}
Pomembna lastnost podatkovne baze, ki zagotavlja toleranco na napake\DIFaddbegin \DIFadd{, }\DIFaddend je tudi obnova podatkovne baze po izpadu. Poleg same obnove sistema, kot ga poznamo pri tradicionalnih podatkovnih bazah, NewSQL podatkovne baze  poizkušajo tudi minimizirati sam čas obnove \cite{Pavlo2016Sep}. Pričakuje se, da bo podatkovna baza na voljo skoraj ves čas, saj že kratkotrajni izpadi lahko pomenijo velike finančne izgube. Tukaj ponavadi govorimo o tako imenovanih petih devetkah (angl. five nines) oziroma 99,999\% razpoložljivostjo, to označimo z pojmom visoka razpoložljivost.

Tradicionalne podatkovne baze ponavadi tečejo na enem vozlišču in \DIFdelbegin \DIFdel{največkrat }\DIFdelend \DIFaddbegin \DIFadd{naj\-več\-krat }\DIFaddend uporabljajo neko implementacijo algoritma ARIES (angl. Algorithms for Recovery and Isolation Exploiting Semantics) za obnovo ob izpadu. ARIES za svoje delovanje uporablja WAL (angl. Write-Ahead Log), to je dnevnik, ki je shranjen na obstojnem pomnilniku, vsaka sprememba pa se najprej zapiše v dnevnik in šele nato izvede. Algoritem ARIES je sestavljen iz treh faz, analize (angl. analysis), ponovitve (angl. redo) in razveljavitev (angl. undo). V fazi analize algoritem pripravi podatkovne strukture. V fazi ponovitve, ponovi vse spremembe zabeležene v WAL dnevniku, v tej točki je podatkovna baza točno v takem stanju kakor je bila pred izpadom. V fazi razveljavitve pa razveljavi vse nepotrjene transakcije. Po končanem postopku je podatkovna baza v konsistentnem stanju. \cite{Pavlo2016Sep}
...
CockroachDB v grobem sestavlja pet funkcionalnih plasti. Zaradi lažjega razumevanja večslojne arhitekture bom najprej predstavil najbolj pogoste termine in koncepte uporabljene v podatkovni baze CockroachDB. Kasneje \DIFdelbegin \DIFdel{pa bom predstavil samo arhitekturo po posameznih plasteh}\DIFdelend \DIFaddbegin \DIFadd{bom povzel samo arhitekturo in delovanje podatkovne, kar je bolj podrobno opisa v uradni dokumentaciji \mbox{%DIFAUXCMD
\cite{CRDB-home} }\hspace{0pt}%DIFAUXCMD
in prvotnih zasnutikh delovanja CockroachDB podatkovne baze \mbox{%DIFAUXCMD
\cite{CRDB-design}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\hline
A$<$\DIFdelbeginFL \DIFdelFL{namen}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{intent}\DIFaddendFL $>$ & 500 & \DIFdelbeginFL \DIFdelFL{"nepotrjena vrednost " }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{nepotrjena vrednost }\DIFaddendFL \\
A & 400 & \DIFdelbeginFL \DIFdelFL{"trenutna vrednost " }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{trenutna vrednost }\DIFaddendFL \\ 
A & 322 & \DIFdelbeginFL \DIFdelFL{"stara vrednost " }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{stara vrednost }\DIFaddendFL \\ 
A & 50 & \DIFdelbeginFL \DIFdelFL{"prvotna vrednost " }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{prvotna vrednost }\DIFaddendFL \\
B & 100 & \DIFdelbeginFL \DIFdelFL{"vrednost B " }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{vrednost B }\DIFaddendFL \\
\hline
\end{tabular}
\end{center}
...
Podatkovna baza CockroachDB privzeto podpira najvišji standardni \DIFdelbegin \DIFdel{ANSI }\DIFdelend \DIFaddbegin \DIFadd{AN\-SI }\DIFaddend SQL izolacijski nivo, to je \textit{serializable}. Ta nivo ne dopušča nikakršnih anomalij v podatkih, če obstaja možnost anomalije se transakcija ponovno izvede. Podpira pa tudi zastareli nestandardni izolacijski nivo \textit{snapshot}.

 
\subsection{Porazdelitvena plast}

Vsi podatki v gruči so na voljo preko \DIFdelbegin \DIFdel{katerega koli }\DIFdelend \DIFaddbegin \DIFadd{kateregakoli }\DIFaddend vozlišča. CockroachDB shranjuje podatke v urejeno vrsto tipa ključ-vrednost. Ta podatkovna \DIFdelbegin \DIFdel{struktura }\DIFdelend \DIFaddbegin \DIFadd{stru\-ktu\-ra }\DIFaddend je namenjena shranjevanju sistemskih kakor tudi uporabniških podatkov. Z nje CockroachDB razbere kje se nahaja podatek in vrednost samega podatka. Podatki so razdeljeni na koščke, katere imenujemo obsegi (angl. ranges). Obsegi so urejeni koščki podatkov preko katerih CockroachDB lahko hitro in učinkovito izvede \textit{lookup} in \textit{scan} operacije.
...
\subsection{SQL}
CockroachDB, kakor večina relacijskih podatkovnih baz, podpira podmnožico ANSI SQL standarda. Poleg tega implementira nekatere najpogostejše \DIFdelbegin \DIFdel{razširitve}\DIFdelend \DIFaddbegin \DIFadd{raz\-ši\-rit\-ve}\DIFaddend , razširitve specifične za \DIFdelbegin \DIFdel{PostgreSQL }\DIFdelend \DIFaddbegin \DIFadd{Postgres }\DIFaddend ter svoje razširitve. V spodnjih podpoglavjih bom v grobem opisal, kaj od jezika SQL ponuja CockroachDB, bolj podroben opis pa se nahaja v uradni dokumentaciji \cite{CRDB-sql-features}.

\subsubsection{Podatkovni tipi}
CockroachDB podpira podatkovne tipe kot so \texttt{ARRAY}, \texttt{BOOL}, \texttt{BOOL}, \texttt{BYTES}, \texttt{COLLATE} (podobno kot \texttt{STRING} vendar upošteva specifike jezika), \texttt{DATE}, \texttt{\DIFdelbegin \DIFdel{DECIMAL}\DIFdelend \DIFaddbegin \DIFadd{DE\-CI\-MAL}\DIFaddend }, \texttt{FLOAT}, \texttt{INET} (IPv4 in IPv6 naslovi), \texttt{INTERVAL} (časovni interval), \texttt{JSONB}, \texttt{SERIAL} (\texttt{INT} z avtomatsko kreirano številko, priporočena uporaba \texttt{UUID}), \texttt{STRING}, \texttt{TIME}, \texttt{TIMESTAMP} in \texttt{UUID}. CockroachDB ne podpira podatkovnih tipov \texttt{XML}, \texttt{UNSIGNED INT} ter \texttt{SET} in \texttt{ENUM}.
...
Uporabniške funkcije niso podprte in tudi ne \DIFdelbegin \DIFdel{planirane }\DIFdelend \DIFaddbegin \DIFadd{načrtovane }\DIFaddend za naslednje verzije. Shranjene procedure so \DIFdelbegin \DIFdel{planirane }\DIFdelend \DIFaddbegin \DIFadd{načrtovane }\DIFaddend za eno od naslednjih verzij.
...
\begin{itemize}
    ...
    \item nadzor dostopa na nivoju skupin \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{in
    }\DIFdelend \DIFaddbegin \DIFadd{in
    }\item \DIFaddend distribuirano kreiranje in obnova varnostnih kopij.
\end{itemize}
...
Podatkovna baza CockroachDB implementira standardni ANSI SQL, za komunikacijo med odjemalcem in strežnikom pa uporablja \DIFdelbegin \DIFdel{PostgreSQL }\DIFdelend \DIFaddbegin \DIFadd{Postgres }\DIFaddend žični protokol, kar omogoča dobro kompatibilnost z obstoječimi \DIFdelbegin \DIFdel{PostgreSQL }\DIFdelend \DIFaddbegin \DIFadd{Postgres }\DIFaddend komponentami.

Na uradni spletni dokumentaciji je za jezike Go, Java, .NET, C++, NodeJS, PHP, Python, Ruby, Rust, Clojure ter Elixir objavljen seznam podprtih gonilnikov in \DIFdelbegin \DIFdel{ORMjev}\DIFdelend \DIFaddbegin \DIFadd{ORM-jev}\DIFaddend . Za te CockroachLabs v času pisanja zagotavlja le beta podporo \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{CRDB-meta-drivers-orms}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

Od orodij za upravljanje z podatkovno bazo nam CockroachLabs ponuja konzolno aplikacijo \texttt{cockroach sql}. Od grafičnih orodij nudijo beta podporo za pgweb, dbglass, Postico, PSequel, TablePlus in Valentina studio \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{CRDB-meta-vizualizers}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Orodje pgweb ima trenutno najbolj zdravo in aktivno skupnost, je odprtokodno in podpira vse glavne operacijske sisteme (Windows, OSX in Linux). Aplikacijo dostopamo preko spletnega brskalnika in podpira funkcionalnosti kot so:
...
%DIFDELCMD < %%%
\DIFdel{Drivers }%DIFDELCMD < & %%%
\DIFdel{ORMs:
https://github. com/cockroachdb/cockroach/issues/25468
}\DIFdelend \DIFaddbegin \DIFadd{Izvirna koda podatkovne baze CockroachDB je javno dostopna preko GitHub repozitorija }\texttt{\DIFadd{cockroachdb/cockraoch}} \DIFadd{\mbox{%DIFAUXCMD
\cite{cockroachdb/cockroach}}\hspace{0pt}%DIFAUXCMD
. Projekt ima zdravo skupnost, kateremu trenutno sledi približno 14 tisoč navdušencev. Poleg GitHub repozitorija, kjer se odvija ves razvoj, vodenje nalog in pomoč, CockroachLabs vodi še spletno stran \mbox{%DIFAUXCMD
\cite{CRDB-home} }\hspace{0pt}%DIFAUXCMD
(dokumentacija, form, blog, mediji), Gitter kanal \mbox{%DIFAUXCMD
\cite{CRDB-gitter} }\hspace{0pt}%DIFAUXCMD
in Docker repozitorij \mbox{%DIFAUXCMD
\cite{CRDB-docker}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 

\DIFdelbegin \DIFdel{DB Visualizers:
https://github.com/cockroachdb/cockroach/issues/25467
}%DIFDELCMD < \fi
%DIFDELCMD < 

%DIFDELCMD < \hl{TODO:} %%%
\DIFdel{skupnost, sam sem uporabil pgweb in gorm
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \chapter{Primerjalna analiza zmogljivosti CockroachDB z Citus}

Primerjalna analiza zmogljivosti (angl. \DIFdelbegin \DIFdel{"performance benchmarking"}\DIFdelend \DIFaddbegin \DIFadd{performance benchmarking}\DIFaddend ) je postopek za primerjavo enega sistema z ostalimi podobnimi sistemi. V mojem primeru bom z orodjem YCSB primerjal podatkovni bazi CockroachDB ter Postgres \DIFdelbegin \DIFdel{(z }\DIFdelend \DIFaddbegin \DIFadd{z nameščeno }\DIFaddend razširitvijo Citus \DIFaddbegin \DIFadd{(v nadeljevanju samo Citus}\DIFaddend ). 

Za primerjavo z \DIFdelbegin \DIFdel{Postgres }\DIFdelend \DIFaddbegin \DIFadd{Citus }\DIFaddend sem se odločil zaradi lažje izvedbe ter bolj primerljivih rezultatov\DIFdelbegin \DIFdel{, saj je CockroachDB v nekaterih vidikih zelo podoben Postgres podatkovni bazi }\DIFdelend \DIFaddbegin \DIFadd{. Obe podatkovni bazi CockroachDB in Citus sta po nekaterih lastnostih zelo podobeni. Obe podatkovni bazi implementirat ANSI SQL ter komunicirata preko Postgres žičnega protokola}\DIFaddend . 

Za orodje YCSB sem se odločil, \DIFdelbegin \DIFdel{ker }\DIFdelend \DIFaddbegin \DIFadd{saj }\DIFaddend podpira vmesnik JDBC\DIFaddbegin \DIFadd{, }\DIFaddend katerega podpirata tudi obe \DIFdelbegin \DIFdel{bazi}\DIFdelend \DIFaddbegin \DIFadd{podatkovni bazi poleg tega pa je enostaven za uporabo}\DIFaddend . Orodje YCSB sem bolj podrobno opisal v poglavju \ref{YCSB_about}. \DIFaddbegin \DIFadd{Ob iskanju najprimernejšega orodja sem pregledal naslednja orodja:
}\DIFaddend 

\DIFaddbegin \begin{itemize}
    \item \textbf{\DIFadd{TPC:}}\\ \DIFadd{TPC (angl. transaction processing performance council) \mbox{%DIFAUXCMD
\cite{TPC-home} }\hspace{0pt}%DIFAUXCMD
je ne profitna organizacija, katera nudi preverljive podatke TPC zmogljivostnih analiz podatkovnih baz. TPC definira več različnih standardnih testov, kateri simulirajo različne realne obremenitve. Te testi so točno opisani in strogo omejeni. Najbolj pozan tip testa za transakcijske obremenitve je TPC-C, za analitične pa TPC-H.
}

    \DIFadd{To so standardizirani testi in bi bili najbolj primerni, vendar pa so zelo kompleksni. Uradnih TPC testov za podatkovno bazo CockraochDB še ne obstaja.
    }\item \textbf{\DIFadd{pgbench:}}\\ \DIFadd{Je enostavno orodje namenjeno Postgres podatkovni bazi. Simulira ne standardno TPC-C obremenitev.
    }

    \DIFadd{Orodje pgbench v času izvajanja testov še ni bilo kompatibilno z podatkovno bazo CockroachDB.
    }\item \textbf{\DIFadd{cockroachdb/loadgen:}}\\ \DIFadd{Je orodje namenjeno CockroachDB podatkovni bazi. Orodje vsebuje nabor testov kot so TPC-C, TPC-H in YCSB. Te orodja interno uporabljajo za zmogljivostno primerjavo med verzijami CockroachDB podatkovne baze. Kasneje je bil objavljena tudi objava na njihovem blogu, kjer so primerjali rezultate TPC-C obremenitve med podatkovnima bazama CockroachDB in Amazon Aurora.
}

    \DIFadd{Orodje je enostavno, vendar ne omogoča zmogljivostne analize Postgres podatkovne baze.
    }\item \textbf{\DIFadd{Apache JMeter:}}\\ \DIFadd{Je orodje primarno namenjeno izvajanju obremenitvenih testov. Tega orodje omogoča izvajanje obremenitvenih testov preko JDBC vmesnika.
    }

    \DIFadd{Orodje je zelo konfigurabilno vendar pa ne omogoča v naprej definiranih obremenitvenih testov in je težko za uporabo.
    }\item \textbf{\DIFadd{YCSB:}}\\ \DIFadd{YCSB (angl. Yahoo! Cloud Serving Benchmark) je orodje namenjeno za primerjavo zmogljivostnih metrik med raznimi podatkovnimi bazami.
}\end{itemize}
...
\begin{enumerate}
    ...
    \iffalse
    Razlog za to hipotezo je, da je CockroachDB na trgu dobro leto in še ni dokončno optimizirana. Medtem, ko je Postgres na trgu dobrih 20 let.
    \fi
...
\subsubsection{\DIFdelbegin \DIFdel{Postgres z razširitvijo }\DIFdelend Citus}
\begin{enumerate}
    \item Z Docker Swarm konfiguracijsko skripto (\texttt{stacks/postgres-n1.yml}) sem pognal \DIFdelbegin \DIFdel{Postgres }\DIFdelend \DIFaddbegin \DIFadd{Citus podatkovno }\DIFaddend bazo na vozlišču \texttt{n2}.
    ...
    \item Nato sem Docker Swarm konfiguracijsko skripto (\texttt{stacks/postgres-n3.yml}) pognal \DIFdelbegin \DIFdel{Postgres }\DIFdelend \DIFaddbegin \DIFadd{Citus podatkovno }\DIFaddend bazo na treh vozliščih.
...
\section{Ugotovitve}
Zmogljivostna analiza je pokazala, da se \DIFdelbegin \DIFdel{Postgres z Citus razširitvijo v }\DIFdelend \DIFaddbegin \DIFadd{Citus pri }\DIFaddend večini \DIFdelbegin \DIFdel{primerov }\DIFdelend \DIFaddbegin \DIFadd{obremenitev, katere sem testiral, }\DIFaddend odziva bolje kot CockroachDB.
...
Druga izjema se je pojavila pri obremenitvi tipa C na enem vozlišču, kjer je CockroachDB dosegel približno eno četrtino večjo prepustnost kot Postgres.

\subsection{Hipoteza 1}
\textit{CockroachDB bo na enem vozlišču nekoliko počasnejši od Postgres podatkovne baze.}\\\\Razlog za to hipotezo je, da je podatkovna baza Postgres že dolgo na trgu \cite{Postgres-first-release} in je uporabljena na mnogih projektih. Zato je zmogljivostno bolje optimizirana kakor CockroachDB. CockroachDB pa je na trgu od leta 2015. Prvo stabilno verzijo (1.0.0) so objavili maja 2017 \cite{CRDB-2017}, druga verzija (2.0.0) pa je bila objavljena aprila 2018. Vsaka verzija je dodala veliko novih funkcionalnosti.
...
\DIFdelbegin \section{\DIFdel{Pregled primerjalnih analiz drugih avtorjev}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \chapter{Sklepne ugotovitve}
\DIFaddbegin 

\DIFadd{V diplomskem delu sem pregledal lastnosti  ter najpogosteje uporabljene mehanizme in pristope pri implementacijah novih arhitektur NewSQL podatkovnih baz. Nato sem opisal arhitekturo in lastnosti izbrane NewSQL podatkovne baze CockroachDB. Podatkovno bazo CockroachDB sem kasneje praktično testiral. Izvedel sem primerjalno analizo zmogljivosti med podatkovno bazo CockroachDB in Postgres (z nameščeno razširitvijo Citus). Veliko časa sem vložil v postavitev testnega okolja, pazil sem, da sem odpravil čim več spremenljivk, ki bi lahko uplivale ne testne scenarije. Testno okolje je bilo sestavljeno z štirih starejših računalnikov, kateri so bili povezani preko gigabitnega omrežja, na njih pa je tekel Ubuntu Server in Docker. Testno okolje sem v začetku nadziral preko Telegraf agenta, InfulxDB podatkovne baze za sehranjevanje časovnih podatkov in Grafane za vizualizacijo podatkov. Kljub trudu, ki je bil vložen v postavitev testnega okolja, bi le tega lahko še izboljšal. Za boljšo primerljivost zmogljivostnih metrik, bi morala imeti vsa vozlišča enako strojno opremo, testirati pa bi moral še konfiguracije z več kot tremi vozlišči. Izvedbo testiranja sem avtomatiziral, z orodjem YCSB sem izvajal različne zmogljivostne teste, razultate pa sem shranjeval v csv datoteko in jih kasneje obdelal z programom Microsoft Excel.
}

\DIFadd{Primerjalna analiza zmogljivosti je pokazala, da je podatkovna baza Cock\-roachDB kljub enostavnim obremenitvam, katere vrši orodje YCSB, v večini primerov slabša od Postgres podatkovne baze. V porvprečju je imel Cock\-roachDB večjo latenco in manjšo prepustnost, poleg tega pa je z večanjem sočasnih povezav zmogljivost padala bistveno hitreje kot pri podatkovni bazi Postgres. Kljub slabšim rezultatov zmogljivostne analize, se moramo zavedati, da je težko pošteno primerjati dve zele različni podatkovni bazi. Podatkovna baza Postgres je bistveno starejša in zato bolj stabilna in optimizirana. CockroachDB pa je na trgu šele dobri dve leti in je trenutno še vedno bolj funkcionalno usmerjena. Za smiselno odločitev pri izbiri podatkovne baze pa moramo poleg zmogljivostnih metrik upoštevati tudi druge, kot so poizvedovalni jezik, podpora transakcijam, zrelost sistema, skupnost, zmogljivost, težavnost postavitve ter vzdrževanja, oslate lastnosti specifične za vsako podatkovno bazo, ...
}

\DIFadd{Podatkovna baza CockroachDB ima močno in aktivno skupnost. Na spletu najdemo že kar nekaj vrednotenj drugih avtorjev, od raznih primerjalnih analiz \mbox{%DIFAUXCMD
\cite{kaur2017performance, Benchmarking-GCS-CRDB-NuoDB, CRDB-tpcc-vs-aurora, CRDB-2017}}\hspace{0pt}%DIFAUXCMD
, do Jepsen testov konsistence in obnašanja sistema med napakami \mbox{%DIFAUXCMD
\cite{CRDB-jepsen, CRDB-jepsen-diy}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{Kljub temu, da je CockroachDB relativno nova podatkovna baza jo nekatera podjetja že uporabljajo. CockroachLabs poizkuša s podjetji sodelovati, nekatera podjetja so tudi prisplevala del funkcionalnosti. Podjetja katera so se odločila za uporabo podatkovne baze CockroachDB, ciljajo globalni trg, poleg tega pa želijo poenostaviti postavitev in vzdrževanje, tako v oblak, kako tudi na svoji infrastrukturi.
}

\begin{itemize}
    \item \textbf{\DIFadd{Baidu}} \DIFadd{uporablja CockroachDB pri dveh novih aplikacijah, ki sta predhodno uporabljale MySQL podatkovno bazo. Ti dve aplikaciji obsegata približno 2TB podatkov in ustvarita prbližno 50M zapisov na dan.
    }\item \textbf{\DIFadd{Kindred}} \DIFadd{se ukvarja z igralništvom, z letom 2014 pa so začeli s prehodom na globalni trg. Imajo kompleksen ekosistem več kot 200 mikrostoritev, ta arhitektura omogoča elastičnost poleg tega pa mora biti skoraj v celoti avtonomna.
    }\item \textbf{\DIFadd{Tierion}} \DIFadd{ponuja "Blockchain proof engine" rešitev. Ovrednotili so več opcij na koncu pa so se odločili za CockroachDB.
    }\item \textbf{\DIFadd{Heroic}} \DIFadd{Labs je hitro rastoč startup, ukvarja se z skalabilnimi zalednimi sistemi v igralniški industriji.
    }\item \textbf{\DIFadd{Gorgias}} \DIFadd{ponuja platformo za pomoč uporabnikom.
}\end{itemize}


%DIF > Podatkovna baza CockroachBD idejno izhaja iz Googlove podatkovne baze Spanner. Podobno je s podatkovno bazo TiDB, katera je v nekaterih pogledih zelo podobna podatkovni bazi CockroachBD. TiDB je prav tako sestavljena z več plasti. Za razliko s CockroachDB, namestitev obsega tri ločene komponente TiDB (vmesnik med SQL in KV operacijami), TiKV (shramba podatkov) in PD (meta podatki in generiranje časovnih oznak).

%DIF > https://groups.google.com/forum/#!topic/tidb-user/k_nMQWPmK-Q

\DIFadd{Glede na znanje katerega sem pridobil skozi diplomsko delo, menim, da je podatkovna baza CockroachDB primerna za nove transakcijsko usmerjene (OLTP) aplikacije, katere zahtevajo visoko konsistenoc in razpoložljivost. Aplikacije katere ciljajo globalni trg, kjer je čas do trga zelo pomemben in kjer si ne moremo privoščiti velikih stroškov vzdrževanja. Pred odločitvijo moramo oceniti še podprto sintakso SQL jezika \mbox{%DIFAUXCMD
...
. Podatkovna baza CockroachDB trenutno ni primerna za scenarije kateri zahtevajo nizko latenco in aplikacije katere so analitično usmerjene (OLAP).
}
...
\hl{TODO:}
\begin{itemize}
    \item \DIFadd{Konkurenčne baze? TiDB, Spanner, ...
    }\item \DIFadd{Kdaj bi bila ta baza primerna?
    }\item \DIFadd{Kaj bi bilo smiselno še raziskati?
}\end{itemize}
\fi
\DIFaddend 
