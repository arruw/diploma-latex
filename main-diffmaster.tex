...
%DIF DEL main-oldtmp-8108.tex   Mon Aug 20 08:15:42 2018
%DIF ADD main.tex               Mon Aug 20 08:14:23 2018
...
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \begin{longtable}{p{0.15\textwidth}|p{.3\textwidth}|p{.5\textwidth}}
    ...
    {\bf \DIFdelbegin \DIFdel{SQL}\DIFdelend \DIFaddbegin \DIFadd{ACID}\DIFaddend }  & \DIFaddbegin \DIFadd{atomicity, consistency, isolation, durability
        }& \DIFadd{atomarnost, konsistentnost, izolacija, trajnost so transakcijske lastnosti katere zagotavljajo relacijske podatkovne baze
        }\\
    {\bf \DIFadd{CSV}}   & \DIFadd{comma-seperated value
        }& \DIFadd{standardni format podatkov ločen z vejico
        }\\
    {\bf \DIFadd{DBaaS}}   & \DIFadd{database as a service
        }& \DIFadd{podatkovna baza, ki je na voljo v oblaku in s katero v večini upravlja ponudnik oblačne storitve 
        }\\
    {\bf \DIFadd{HTAP}}  & \DIFadd{hybrid transaction/analytical processing
        }& \DIFadd{oznaka sistema, ki omogoča tako transakcijsko kakor tudi analitično obdelovanje
        }\\
    ...
    {\bf \DIFadd{JSON}}  & \DIFadd{Javascript object notation
        }&  \DIFadd{standardna format za prenos podatkov v spletu}\\
    {\bf \DIFadd{KV}}    & \DIFadd{key-value
        }& \DIFadd{ključ-vrednost }\\
    {\bf \DIFadd{MVCC}}    & \DIFadd{multiversion concurrency control
        }& \DIFadd{mehanizem za nadzor sočasnosti, kateri hrani več verzij podatkov, omogoča izolacijo in konsistenco podatkov med transakcijami }\\
    ...
    {\bf NoSQL} & not \DIFaddbegin \DIFadd{only }\DIFaddend structured query language
        & \DIFdelbegin \DIFdel{ne strukturiran poizvedovalni jezik }\DIFdelend \DIFaddbegin \DIFadd{poizvedovalni jezik za delo z nerelacijskimi podatki v nekaterih primerih tudi relacijskimi
        }\DIFaddend \\
    {\bf \DIFdelbegin \DIFdel{YCSB}\DIFdelend \DIFaddbegin \DIFadd{NTP}\DIFaddend } & \DIFdelbegin \DIFdel{Yahoo! Cloud Serving Benchmark        }\DIFdelend \DIFaddbegin \DIFadd{network time protocol
        }\DIFaddend & \DIFdelbegin \DIFdel{Yahoo! Cloud Serving Benchmark }\DIFdelend \DIFaddbegin \DIFadd{meržni protokol za sinhronizacijo ure med računalniškimi sistemi
        }\DIFaddend \\
    {\bf \DIFdelbegin \DIFdel{JDBC}\DIFdelend \DIFaddbegin \DIFadd{OLAP}\DIFaddend }  & \DIFdelbegin \DIFdel{Java Database Connectivity            }\DIFdelend \DIFaddbegin \DIFadd{online analytical processing
        }\DIFaddend & \DIFdelbegin \DIFdel{javanski vmesnik za povezavo s }\DIFdelend \DIFaddbegin \DIFadd{oznaka sistema, ki omogoča analitično obdelovanje podatkov
        }\\
    {\bf \DIFadd{OLTP}}  & \DIFadd{online transaction processing
        }& \DIFadd{oznaka sistema, ki omogoča obdelavo transakcij
        }\\
    ...
    {\bf \DIFadd{SQL}}   & \DIFadd{structured query language    
        }& \DIFadd{strukturiran povpraševalni jezik za delo s relacijskimi }\DIFaddend podatkovnimi bazami
        \\
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin {\bf \DIFadd{WAL}}   & \DIFadd{write-ahead logging
        }& \DIFadd{mehanizem zabeleži najprej vse spremembe v dnevnik šele na to pa jih izvede, mehanizem zagotavlja atomarnost in trajnost podatkov 
        }\\
    {\bf \DIFadd{XML}}   & \DIFadd{extensible markup language
        }& \DIFadd{format za izmenjavo strukturiranih podatkov v spletu
        }\\
    {\bf \DIFadd{YCSB}}  & \DIFadd{Yahoo! Cloud Serving Benchmarking
        }& \DIFadd{enostavno orodje za izvedbo primerjalne zmogljivostne analize med različnimi podatkovnimi bazami
        }\\
    {\bf \DIFadd{2PL}} & \DIFadd{two-phase locking
        }& \DIFadd{mehanizem za nadzor sočasnosti, kateri preko zaklepanja zagotavlja serializabilnost transakcij
        }\\
\end{longtable}
\DIFaddend 
...
\DIFaddbegin \noindent \DIFaddend Lastnosti NewSQL podatkovnih baz so:
\begin{itemize}
    ...
    \item Uporabljajo \DIFdelbegin \DIFdel{ne blokajoče }\DIFdelend \DIFaddbegin \DIFadd{neblokirajoče }\DIFaddend mehanizme za nadzor sočasnosti. Tako pisalni zahtevki ne povzročajo konfliktov z bralnimi zahtevki \DIFdelbegin \DIFdel{.
    }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{NewSQLNewWayToHandleBigData}}\hspace{0pt}%DIFAUXCMD
. To smo podrobneje opisali v poglavju \ref{newsql/concurrencycontroll}.
    }\DIFaddend \item Implementirajo distribuirano ne deljeno (angl. shared-nothing) arhitekturo. Ta arhitektura zagotavlja horizontalno skalabilnost, podatkovna baza pa lahko teče na velikem številu vozlišč brez trpljenja ozkih grl \cite{NewSQLNewWayToHandleBigData}.
    \item Omogočajo dosti boljšo zmogljivost \DIFdelbegin \DIFdel{na enem vozlišču }\DIFdelend v primerjavi z tradicionalnimi relacijskimi podatkovnimi bazami. Nekateri pravijo, da naj bi bile NewSQL podatkovne baze celo 50 krat bolj zmogljive \cite{Kumar2018Jun}.
\end{itemize}
...
\cite{Pavlo2016Sep}
    ...
    }\DIFaddend \item \textbf{Transparentno drobljenje:}\\To so komponente, ki usmerjajo poizvedbe, koordinirajo transakcije, upravljajo s podatki, particijami ter replikacijo. Prednost teh komponent je, da največkrat ni potrebno prilagajati aplikacije novi arhitekturi, saj še vedno deluje kot ena logična podatkovna baza. \DIFaddbegin \DIFadd{Slabost teh arhitektur pa se odraža predvsem pri neenakomerno porazdeljenih podatkih. }\DIFaddend Primer takega transparentnega vmesnika je MariaDB MaxScale.
    \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Pavlo2016Sep}
    }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \item \textbf{Novi shranjevalni \DIFdelbegin \DIFdel{motorji }\DIFdelend \DIFaddbegin \DIFadd{pogoni }\DIFaddend in razširitve:}\\V to kategorijo spadajo novi shranjevalni \DIFdelbegin \DIFdel{motorji }\DIFdelend \DIFaddbegin \DIFadd{pogoni }\DIFaddend in razširitve, kateri poizkušajo rešiti probleme skaliranja tradicionalnih relacijskih podatkovnih baz. \cite{Kumar2018Jun} Primer rešitev, ki sodijo v to kategorijo so na primer MySQL NDB cluster, Citus razširitev za \DIFdelbegin \DIFdel{Postgres }\DIFdelend \DIFaddbegin \DIFadd{PostgreSQL }\DIFaddend podatkovno bazo ter Hekaton OLTP motor za SQL Server. \DIFdelbegin \DIFdel{Glede na neke vire }\DIFdelend \DIFaddbegin \DIFadd{Podatkovno bazo Citus smo bolj podrobno opisali v poglavju \ref{benchmarking/citus}. Glede na }\DIFaddend \cite{Pavlo2016Sep} te rešitve ne sodijo v svet NewSQL podatkovnih baz\DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{, ampak le med razširitve repacijskih SQL podatkovnih baz. Slabost teh v primerjavi z novimi arhitekturami je predvsem slabša izraba računalniških resursov. Poleg tega pa so arhitekture veliko manjn elastične in bollj funkionalno omejene.
}\DIFaddend \end{enumerate}

\section{Nadzor sočasnosti}
\DIFaddbegin \label{newsql/concurrencycontroll}
\DIFaddend Nadzor sočasnosti je en izmen najpomembnejših mehanizmov v podatkovnih bazah. Omogoča, da do podatkov dostopa več niti hkrati, med tem pa ohranja atomičnost ter garantira izolacijo \cite{Pavlo2016Sep}.
...
\begin{figure}[H]
    ...
    \end{center}
    \caption{\DIFaddFL{Groba primerjava povprečne prepustnosti in latence med obema podatkovnima bazama na enem in treh vozliščih.}}
...
}\DIFaddend ). Veliko časa \DIFdelbegin \DIFdel{sem vložil }\DIFdelend \DIFaddbegin \DIFadd{smo vložili }\DIFaddend v postavitev testnega okolja \DIFdelbegin \DIFdel{, pazil sem, da sem odpravil }\DIFdelend \DIFaddbegin \DIFadd{in bili pozorni, da smo odpravili }\DIFaddend čim več spremenljivk, ki bi lahko \DIFdelbegin \DIFdel{uplivale ne }\DIFdelend \DIFaddbegin \DIFadd{vplivale na }\DIFaddend testne scenarije. Testno okolje je bilo sestavljeno \DIFdelbegin \DIFdel{z }\DIFdelend \DIFaddbegin \DIFadd{s }\DIFaddend štirih starejših računalnikov, kateri so bili povezani preko gigabitnega omrežja, na njih pa je tekel Ubuntu Server \DIFdelbegin \DIFdel{in Docker}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ubuntu-server} }\hspace{0pt}%DIFAUXCMD
in Docker \mbox{%DIFAUXCMD
\cite{docker}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Testno okolje \DIFdelbegin \DIFdel{sem }\DIFdelend \DIFaddbegin \DIFadd{smo }\DIFaddend v začetku \DIFdelbegin \DIFdel{nadziral }\DIFdelend \DIFaddbegin \DIFadd{nadzirali }\DIFaddend preko Telegraf agenta \DIFdelbegin \DIFdel{, InfulxDB }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
