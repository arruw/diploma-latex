...
%DIF DEL main-oldtmp-11852.tex   Fri Aug 31 09:01:09 2018
%DIF ADD main.tex                Fri Aug 31 08:58:02 2018
...
%DIF -------
\newcommand{\ttitle}{Visoko skalabilen NewSQL sistem za upravljanje s podatkovnimi bazami CockroachDB} %DIF > 
\newcommand{\ttitleEn}{Highly scalable NewSQL database management system CockroachDB} %DIF > 
%DIF -------
...
\end{tabbing}
\DIFdelbegin \DIFdel{Besedilo teme diplomskega dela študent prepiše iz študijskega informacijskega sistema, kamor ga je vnesel mentor. V nekaj stavkih bo opisal, kaj pričakuje od kandidatovega diplomskega dela.
Kaj so cilji, kakšne metode uporabiti, morda bo zapisal tudi ključno literaturo.
}\DIFdelend \DIFaddbegin \DIFadd{Preglejte in opišite osnovne koncepte, uporabljene v NewSQL sistemih za upravljanje s podatkovnimi bazami in jih predstavite na primeru sistema CockroachDB. Izvedite primerjalno analizo lastnosti in zmogljivosti med NewSQL sistemom CockroachDB ter tradicionalnim relacijskim sistemom PostgreSQL z razširitvijo Citus.
}\DIFaddend \vspace{15mm}
...
\noindent\begin{longtable}{p{0.15\textwidth}|p{.3\textwidth}|p{.5\textwidth}}
    ...
    {\bf ORM}   & object-relational mapper
        ...
        \\
    %DIF >  {\bf SQL}   & structured query language    
    %DIF >      & strukturiran povpraševalni jezik za delo z relacijskimi podatkovnimi bazami
    %DIF >      \\
    {\bf \DIFdelbegin \DIFdel{SQL}%DIFDELCMD < }   & %%%
...
V tretjem poglavju bomo opisali celoten postopek izvedbe primerjalne analize zmogljivosti med podatkovno bazo CockroachDB in \DIFdelbegin \DIFdel{Citus \mbox{%DIFAUXCMD
\cite{citus}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{PostgreSQL (z nameščeno razširitvijo Citus \mbox{%DIFAUXCMD
\cite{citus}}\hspace{0pt}%DIFAUXCMD
)}\DIFaddend . Opisali bomo hipoteze, testno infrastrukturo, izbiro orodij za izvedbo zmogljivostnih analiz, \DIFdelbegin \DIFdel{podatkovno bazo }\DIFdelend \DIFaddbegin \DIFadd{razširitev }\DIFaddend Citus in razlog\DIFaddbegin \DIFadd{, }\DIFaddend za kaj smo CockroachDB primerjali ravno z njo, pripravo podatkov \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{ter }\DIFaddend izvedbo analize. Na koncu bomo predstavili rezultate \DIFaddbegin \DIFadd{ter }\DIFaddend jih komentirali.
...
\cite{Kumar2018Jun}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend Glede na \cite{Pavlo2016Sep} te rešitve ne sodijo v svet NewSQL podatkovnih baz, ampak le med razširitve relacijskih SQL podatkovnih baz. Slabost teh v primerjavi z novimi arhitekturami je predvsem slabša izraba računalniških resursov. Poleg tega pa so arhitekture veliko manj elastične in bolj funkionalno omejene. Primeri rešitev, ki sodijo v to kategorijo\DIFaddbegin \DIFadd{, }\DIFaddend so na primer MySQL NDB cluster \cite{mysqlndbcluster-home}, Citus \cite{citus} razširitev za PostgreSQL podatkovno bazo, Hekaton \cite{hekaton-home} OLTP pogon za SQL Server \DIFdelbegin \DIFdel{, }\DIFdelend ... \DIFdelbegin \DIFdel{Podatkovno bazo }\DIFdelend \DIFaddbegin \DIFadd{Razširitev }\DIFaddend Citus smo bolj podrobno opisali v poglavju \ref{benchmarking/citus}.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{\DIFaddbegin \DIFadd{Plast }\DIFaddend SQL\DIFdelbegin \DIFdel{plast}\DIFdelend :} Prevede SQL poizvedbe v operacije tipa ključ-vrednost (KV).
...
\subsection{\DIFaddbegin \DIFadd{Plast }\DIFaddend SQL\DIFdelbegin \DIFdel{plast}\DIFdelend }

\DIFdelbegin \DIFdel{SQL plast }\DIFdelend \DIFaddbegin \DIFadd{Plast SQL }\DIFaddend predstavlja vmesnik med podatkovno bazo ter ostalimi aplikacijami.  Podatkovna baza CockroachDB implementira velik del SQL standarda \cite{CRDB-sql-standard}. Zunanje aplikacije komunicirajo preko PostgreSQL žičnega protokola (angl. PostgreSQL wire protocol) \cite{psql-wire-protocol}. To omogoča enostavno povezavo med zunanjimi aplikacijami in kompatibilnost z obstoječimi gonilniki, orodji ter ORM-ji.
...
Za vsak obseg obstaja \DIFdelbegin \DIFdel{Raft skupina }\DIFdelend \DIFaddbegin \DIFadd{skupina Raft}\DIFaddend , kjer je eno vozlišče, ki vsebuje repliko, \DIFdelbegin \DIFdel{označena }\DIFdelend \DIFaddbegin \DIFadd{označeno }\DIFaddend kot "vodja". Vodja je izvoljen in koordinira vse pisalne zahtevke za določen obseg. V idealnih pogojih je vodja \DIFdelbegin \DIFdel{Raft skupine }\DIFdelend \DIFaddbegin \DIFadd{skupine Raft }\DIFaddend tudi najemnik obsega (angl. leaseholder).
...
\begin{itemize}
    ...
    \item \textbf{Ostali}: \texttt{ARRAY}, \texttt{BOOL}, \texttt{BYTES}, \texttt{INET} (IPv4 in IPv6 naslovi), \texttt{JSONB} \DIFdelbegin \DIFdel{in}%DIFDELCMD < \\%%%
\DIFdelend \DIFaddbegin \DIFadd{(podatki v JSON obliki, brez presledkov in podvojenih ključev, predstavljeni v binarnem zapisu) in }\DIFaddend \texttt{UUID} (\DIFdelbegin \DIFdel{128 bitna }\DIFdelend \DIFaddbegin \DIFadd{128-bitna }\DIFaddend heksadecimalna vrednost)
\end{itemize}
...
Primerjalna analiza zmogljivosti (angl. performance benchmarking) je postopek za primerjavo enega sistema z ostalimi podobnimi sistemi. V našem primeru smo z orodjem YCSB primerjali podatkovni bazi CockroachDB ter PostgreSQL z nameščeno razširitvijo Citus (v nadaljevanju \DIFaddbegin \DIFadd{tudi }\DIFaddend samo Citus). 
...
Ker tehnologije Docker še nismo dobro poznali, smo imeli v fazi prototipiranja težave pri zmogljivosti. Izkazalo se je, da je bil ključen problem v tem, da \DIFdelbegin \DIFdel{nismo uporabi podatkovnih nosilcov }\DIFdelend \DIFaddbegin \DIFadd{sprva nismo uporabili podatkovnih nosilcev }\DIFaddend (angl. data volumes) \cite{docker-storage-layers}.
...
Citus je razširitev za podatkovno bazo PostgreSQL \cite{citus}. Omogoča enostavno horizontalno skaliranje podatkov za \DIFdelbegin \DIFdel{več najemniške }\DIFdelend \DIFaddbegin \DIFadd{večnajemniške }\DIFaddend (angl. multi tenant) aplikacije in obdelavo analitičnih podatkov v realnem času (angl. real time analytics). Uporaba razširitve Citus ni primerna\DIFaddbegin \DIFadd{, }\DIFaddend če:
\begin{itemize}
...
Orodje YCSB vrača rezultate v enostavni tekstovni obliki. Od rezultatov vrne skupni čas trajanja, \DIFdelbegin \DIFdel{prepustnosti }\DIFdelend \DIFaddbegin \DIFadd{prepustnost }\DIFaddend (enačba \ref{eq:throughput})\DIFdelbegin \DIFdel{ter agregirano }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend število vseh operacij in različne podatke o latencah (\DIFaddbegin \DIFadd{latenca se meri na nivoju vsake operacije posebaj; }\DIFaddend enačba \ref{eq:latency}). Od latence vrača povprečno vrednost, minimalno in maksimalno vrednost \DIFdelbegin \DIFdel{, }\DIFdelend ter 95 in 99 percentil.
...
Citus 3 \DIFdelbeginFL \DIFdelFL{vozlišče        }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{vozlišča        }\DIFaddendFL & 163,049 \\
CockroachDB 1 vozlišče  & (prekinjeno po 1 minuti)\\
CockroachDB 3 \DIFdelbeginFL \DIFdelFL{vozlišče  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{vozlišča  }\DIFaddendFL & (prekinjeno po 1 \DIFdelbeginFL \DIFdelFL{minutu}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{minuti}\DIFaddendFL )\\
\end{tabular}
\end{center}
...
\end{center}
\caption{Prikazuje primerjavo prepustnosti in latenc glede na vrsto obremenitve med podatkovno bazo \DIFdelbeginFL \DIFdelFL{Citus in }\DIFdelendFL CockroachDB \DIFaddbeginFL \DIFaddFL{in Citus}\DIFaddendFL . Graf (7) je zaradi lažje primerjave odrezan. \DIFdelbeginFL \DIFdelFL{Z }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Iz }\DIFaddendFL grafov \DIFdelbeginFL \DIFdelFL{se vidi}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{je razvidno}\DIFaddendFL , da je \DIFdelbeginFL \DIFdelFL{podatkovna baza }\DIFdelendFL Citus v primerjavi \DIFdelbeginFL \DIFdelFL{z }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{s podatkovno bazo }\DIFaddendFL CockroachDB veliko bolj \DIFdelbeginFL \DIFdelFL{stabilna}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{stabilen}\DIFaddendFL , kar se odraža v razponu med največjo in najmanjšo vrednostjo.}
...
\section{Ugotovitve}
Zmogljivostna analiza je pokazala, da se \DIFdelbegin \DIFdel{podatkovna baza }\DIFdelend Citus pri večini obremenitev, katere \DIFdelbegin \DIFdel{sem testiral, odziva bitveno }\DIFdelend \DIFaddbegin \DIFadd{smo testirali, odziva bistveno }\DIFaddend bolje kot podatkovna baza CockroachDB.
...
S slike \ref{img_ycsb_results_bnw_comparison} je razvidno, da je podatkovna baza CockroachDB v primerjavi s \DIFdelbegin \DIFdel{podatkovno bazo }\DIFdelend Citus manj stabilna. To se odraža na posameznih grafih\DIFdelbegin \DIFdel{saj so podatki }\DIFdelend \DIFaddbegin \DIFadd{, saj so rezultati meritev }\DIFaddend pri podatkovni bazi CockroachDB veliko bolj razpršeni.

Če primerjamo spremembe sočasnih niti, pri katerih sta bazi dosegali maksimalno prepustnost (slika \ref{img_ycsb_results_max_throughput})\DIFaddbegin \DIFadd{, }\DIFaddend opazimo naslednje. Podatkovna baza CockroachDB ob skaliranju na tri vozlišča \DIFaddbegin \DIFadd{lahko }\DIFaddend v povprečju \DIFdelbegin \DIFdel{lahko }\DIFdelend obdela 5,4 več sočasnih niti, \DIFdelbegin \DIFdel{podatkovna baza }\DIFdelend Citus pa kar 10,8 manj.
...
%DIFDELCMD < %%%
\DIFdel{To hipotezo sem ovrgel}\DIFdelend \DIFaddbegin \DIFadd{To hipotezo smo ovrgli}\DIFaddend . Meritve so pokazale, da podatkovna baza CockroachDB doseže celo slabšo prepustnost na treh vozliščih. To \DIFdelbegin \DIFdel{razvidno na sliki }\DIFdelend \DIFaddbegin \DIFadd{je razvidno iz slike }\DIFaddend \ref{img_ycsb_results_top_level_comparison}.
...
Na podlagi teh ugotovitev \DIFdelbegin \DIFdel{sem pripravil }\DIFdelend \DIFaddbegin \DIFadd{smo pripravili }\DIFaddend optimistično oceno rasti povprečne prepustnosti. Obe bazi se v idealnih pogojih in do neke mere skalirata linearno, kar je prikazano na sliki \ref{img_ycsb_results_scaling_throughptu_prediction}. Vendar pa se pri PostgreSQL konfiguraciji vsi odjemalci \DIFdelbegin \DIFdel{direktno }\DIFdelend \DIFaddbegin \DIFadd{neposredno }\DIFaddend povezujejo na eno vozlišče (koordinatorja), \DIFdelbegin \DIFdel{to vozlišče }\DIFdelend \DIFaddbegin \DIFadd{ki }\DIFaddend je ozko grlo (točka A), saj bo ob \DIFdelbegin \DIFdel{večanjem }\DIFdelend \DIFaddbegin \DIFadd{večanju }\DIFaddend v neki točki prišlo do zasičenosti resursov \cite{Citus-add-coordinator}. V neki točki (točka B) pa bo CockroachDB presegel prepustnost podatkovne baze \DIFaddbegin \DIFadd{PostgreSQL z nameščeno razširitvijo }\DIFaddend Citus.

\begin{figure}[H]
\begin{center}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\textwidth]{resources/scaling-throughput-prediction-v2.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.7\textwidth]{resources/scaling-throughput-prediction-v2.png}
\DIFaddendFL \end{center}
\caption{Optimistična ocena rasti povprečne prepustnosti. Pri \DIFdelbeginFL \DIFdelFL{podatkovni bazi }\DIFdelendFL Citus \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL se v točki A linearna rast konča. V točki B pa \DIFdelbeginFL \DIFdelFL{je }\DIFdelendFL ima CockroachDB že večjo prepustnost. Graf je le simboličen in ne predstavlja realnih meritev.}
